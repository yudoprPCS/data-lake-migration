...
def grand_loop(global_param=None, specify_path=None, input_year=CurrDate().year_int(), input_month=CurrDate().month_int(), input_day=CurrDate().day_int(), predict_x_days=70, batch_size=1000, step_start=1):
    ...
    # BASE_DIR = '/project/data/thermal_paper_prediction'
    # file_path = f'{BASE_DIR}/{steps_dir_name}/'
    # df_final = concat_dfs(path=file_path)
...
def transform_grand_loop(sum_choice=50, one_roll_total_cm=1150, cm_trx_with_brimo=14.7, cm_trx_with_no_brimo=10, cm_stl=36, cm_gqr=13.5, default_pm_needs_rolls=1, default_print_behavior=3, default_reprint_behavior=0, max_of_reprint_behavior=0.5):
    ...
    df_transformed.to_csv("/project/data/thermal_paper_prediction/ready_to_insert_to_elk/transformed.csv", sep=";", index=False)
...
def populate_index(index_name, path, insert_batch_size=10, insert_step_start=1):
    ...
    elif path == '/project/data/thermal_paper_prediction/ready_to_insert_to_elk/transformed.csv':
        df['installed_date'] = pd.to_datetime(df['installed_date'], format='ISO8601')
        df['installed_date'] = df['installed_date'] - pd.Timedelta(hours=7)
...
if __name__ == "__main__":
    ...
    get_global_trend(global_param="settlement_count")

    grand_loop(
        global_param="settlement_count",
        specify_path=args.specify_path,
        step_start=args.step_start
    )
    
# grand_loop(global_param="settlement_count", step_start=1, specify_path="thermal_paper_lte_20251218_sum_area_20260101_to_20260131")
# grand_loop(global_param="settlement_count", input_year=
# grand_loop(global_param=None, specify_path=None, input_year=CurrDate().year_int(), input_month=CurrDate().month_int(), input_day=CurrDate().day_int(), predict_x_days=70, batch_size=1000, step_start=1)