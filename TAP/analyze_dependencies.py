"""Analyze TAP (PCSDatahub) dependency graph: parse CSV, rank by children count, generate visual graph."""
...
CSV_PATH = os.path.join(SCRIPT_DIR, "Migrations Plan - ELK & PostgreSQL PCSDatahub.csv")
GRAPH_PNG = os.path.join(SCRIPT_DIR, "tap_dependency_graph.png")
MERMAID_MD = os.path.join(SCRIPT_DIR, "tap_dependency_graph.md")
...
def print_priority_table(G):
    ...
    tables = sum(1 for _, d in G.nodes(data=True) if d['type'] == 'Table')
    procedures = sum(1 for _, d in G.nodes(data=True) if d['type'] == 'Procedure')
    views = sum(1 for _, d in G.nodes(data=True) if d['type'] == 'View')

    print(f"\n Summary: {total_nodes} nodes ({dashboards} Dashboards, {indices} Indices/Transforms, {tables} Tables, {procedures} Procedures, {views} Views), {total_edges} edges")
...
def generate_graph_png(G, output_path):
    ...
    node_colors = []
    for n in G.nodes():
        node_colors.append(color_map.get(G.nodes[n]['type'], '#95A5A6'))
    ...
    legend_elements = [
        ...
        mpatches.Patch(color='#2ECC71', label='Table'),
        mpatches.Patch(color='#9B59B6', label='Procedure'),
        mpatches.Patch(color='#1ABC9C', label='View'),
    ]
    ax.legend(handles=legend_elements, loc='upper left', fontsize=12)
    ax.set_title('TAP (PCSDatahub) ELK & PostgreSQL -> Data Lake Dependency Graph\n(Node size = migration priority / dependents count)',
